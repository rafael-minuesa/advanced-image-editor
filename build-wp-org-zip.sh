...
PLUGIN_NAME="advanced-image-editor"
ZIP_NAME="${PLUGIN_NAME}.zip"
ZIP_PATH="$PARENT_DIR/$ZIP_NAME"

echo -e "${GREEN}Building WordPress.org submission ZIP for Advanced Image Editor...${NC}"
echo ""

# Check if we're in the right directory
if [ ! -f "$PLUGIN_DIR/advanced-image-editor.php" ]; then
    echo -e "${RED}Error: advanced-image-editor.php not found. Please run this script from the plugin directory.${NC}"
    exit 1
fi
